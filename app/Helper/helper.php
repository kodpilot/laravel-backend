...
{
    $permissions = permissions::where('menuStatus', '1')->where('role_id', Auth::user()->admin)->where('menu_id', $menu_id)->count();
    if ($permissions > 0) {
        return true;
    } else {
        return false;
...
function subMenuPermission($menu_id)
{
    ...
    $permissions = permissions::where('menuStatus', '2')->where('role_id', Auth::user()->admin)->where('menu_id', $menu_id)->count();
    if ($permissions > 0) {
        return true;
    } else {
        return false;
