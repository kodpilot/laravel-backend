...
class mobileAPIController extends Controller
{
    ...
    public function cvPage(Request $request){
        ...
        $cvs = cv_infos::select(
            ...
            'cv_infos.profile_file as profile_file',
    ...
    public function cvAdd(Request $request){
        ...
        if ($request->hasFile('profile_file')) {
            try {
                $profile_file = uniqid().'.'.$request->file('profile_file')->getClientOriginalExtension();
                $request->file('profile_file')->move(public_path('/assets/cv/') ,$profile_file);
                $path= public_path('/assets/cv/').$profile_file;
                ImageOptimizer::optimize($path);
            } catch (\Throwable $th) {
                \Log::error($th);
                return response(['status'=>0,'message'=>"unexcepted error occured when file save"]);
        ...
        if ($profile_file == "") {
            return response(['status'=>0,'message'=>"profile image is required"]);
        }
    ...
    public function cvEdit(Request $request , $cv_id = null){
        ...
        if ($request->hasFile('profile_file')) {
            try {
                ...
                \Log::error($th);
                return response(['status'=>0,'message'=>"unexcepted error occured when profile image save"]);
            }
        }
        $cv->name = $name;
        $cv->video = $video;
